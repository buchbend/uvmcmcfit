def plotPDF(fitresults, tag, limits="", Ngood=5000, axes="auto"):
    ...
    # plotting parameters
    rc("font", **{"family": "sans-serif", "sans-serif": ["Arial Narrow"], "size": "12"})
...
def plotImage(model, data, config, modeltype, fitresult, tag=""):
    ...
    plt.subplots_adjust(left=0.08, right=0.97, top=0.97, bottom=0.08, wspace=0.35)
    paramData = setuputil.loadParams(config)
    nlensedsource = paramData["nlensedsource"]
    nlensedregions = paramData["nlensedregions"]
    npar_previous = 0
    ...
    for iregion in range(nregion):
        ...
        for i in range(nsource):
            ...
            if list(config.keys()).count("xtextoff") > 0:
                xytext = (xxx + config["xtextoff"][i], yyy + config["ytextoff"][i])
                if iregion == 0:
                    if modeltype != "optical":
                        plt.annotate(
                            str(i), xy=(xxx, yyy), xytext=xytext, fontsize="xx-large"
                        )
                else:
                    xytext = (
                        xxx + config["xtextoff"][i + 2],
